...
bushes = [int(bush) for bush in input("Введите количество ягод на каждом кусте через пробел: ").split()]

max_berries = bushes[-2] + bushes[-1] + bushes[0]

for i in range(len(bushes) - 1):
    if bushes[i - 1] + bushes[i] + bushes[i + 1] > max_berries:
        max_berries = bushes[i - 1] + bushes[i] + bushes[i + 1]

print(f'Максимальное число ягод будет равно: {max_berries}')
