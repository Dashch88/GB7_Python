# 3) Уставшие от необычно теплой зимы, жители решили узнать, действительно ли это самая длинная оттепель за всю историю наблюдений за погодой.
# Они обратились к синоптикам, а те, в свою очередь, занялись исследованиями статистики за прошлые годы. Их интересует, сколько дней
# длилась самая длинная оттепель. Оттепелью они называют период, в который среднесуточная температура ежедневно превышала 0 градусов Цельсия.
# Напишите программу, помогающую синоптикам в работе.

# Пользователь вводит число N – общее количество рассматриваемых дней (1 ≤ N ≤ 100). В следующих строках располагается N целых чисел.

# Каждое число – среднесуточная температура в соответствующий день. Температуры – целые числа и лежат в диапазоне от –50 до 50

# n = 10
# 10 1 2 -1 2 4 2 5 5 -3

# вывод 5


import os
os.system('clear')

days_amount = int(input('Введите рассматриваемое количество дней: '))
count_warm_days = 0
max_count_warm_days = 0

for i in range(days_amount):
    if int(input(f'Введите среднесуточную температуру {i+1}-го дня: ')) > 0:
        count_warm_days += 1
        if count_warm_days > max_count_warm_days:
            max_count_warm_days = count_warm_days
    else:
        count_warm_days = 0
print(f'Самый длинный период оттепели составил {max_count_warm_days} дней.')